  ...
  commands.addCommand(CommandIDs.downloadGeoJSON, {
    ...
    isEnabled: () => {
      ...
      return selectedLayer
        ? ['VectorLayer', 'ShapefileLayer', 'VectorTileLayer'].includes(selectedLayer.type)
        : false;
    },
    execute: async () => {
      const selectedLayer = getSingleSelectedLayer(tracker);
      ...
      const geojsonString = await getGeoJSONDataFromLayerSource(selectedLayer, source, model);
      if (!geojsonString) {
        return;
      }
