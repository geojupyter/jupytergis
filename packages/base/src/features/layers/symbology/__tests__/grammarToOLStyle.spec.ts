...
describe('grammarToOLStyle — text encodings', () => {
  it('outputs a literal string on a text encoding', () => {
    const style = grammarToOLStyle(
      makeState({
        id: '1',
        mappings: [
    ...
    expect(style['text-font']).toEqual('12px sans-serif');
  });

  it('labels features from a field, coercing non-string values', () => {
    const style = grammarToOLStyle(
      makeState({
        id: '1',
        fields: ['name'],
    ...
    // Absent attributes render no label rather than throwing in ['to-string'].
    expect(style['text-value']).toEqual([
      'case',
      ['has', 'name'],
      [
        'string',
        ['get', 'name'],
        ['to-string', ['number', ['get', 'name'], 0]],
      ],
      '',
    ]);
  });

  it('falls back to the empty string, not 0, when no rule matches', () => {
    const style = grammarToOLStyle(
      makeState({
        id: '1',
        fields: ['kind'],
        when: [{ type: 'fieldEquals', field: 'kind', value: 'city' }],
        mappings: [
          {
            scale: { scheme: 'constant_str', params: { value: 'City' } },
            encodings: ['text-value'],
          },
        ],
      }),
    );
    // ['case', guard, 'City', <else>]
    expect((style['text-value'] as unknown[])[3]).toEqual('');
  });

  it('drives label colour and halo through the usual colour and number scales', () => {
    const style = grammarToOLStyle(
      makeState({
        id: '1',
        mappings: [
    ...
    expect(style['text-fill-color']).toEqual([0, 0, 0, 1]);
    expect(style['text-stroke-width']).toEqual(2);
  });
});
