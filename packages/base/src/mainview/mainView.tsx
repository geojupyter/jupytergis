...
export class MainView extends React.Component<IProps, IStates> {
	constructor(props: IProps) {
		...
		this._mainViewModel.viewSettingChanged.connect(
			this._onViewChanged,
			this,
		);
		...
		this._model.sharedLayerTreeChanged.connect(
			this._onLayerTreeChange,
			this,
		);
		this._model.sharedSourcesChanged.connect(
			this._onSourcesChange,
			this,
		);
		this._model.sharedModel.changed.connect(
			this._onSharedModelStateChange,
		);
		...
		this._model.zoomToPositionSignal.connect(
			this._onZoomToPosition,
			this,
		);
		this._model.updateLayerSignal.connect(
			this._triggerLayerUpdate,
			this,
		);
		this._model.addFeatureAsMsSignal.connect(
			this._convertFeatureToMs,
			this,
		);
	...
	componentWillUnmount(): void {
		...
		this._model.themeChanged.disconnect(
	...
	async generateMap(center: number[], zoom: number): Promise<void> {
		if (this.divRef.current) {
			...
			dragAndDropInteraction.on('addfeatures', event => {
				...
				this._model.sharedModel.addSource(
					sourceId,
					sourceModel,
				);
			...
			if (
				...
				await this._updateLayersImpl(
					JupyterGISModel.getOrderedLayerIds(
						this._model,
					),
				);
				const options = this._model.getOptions();
	...
	createSelectInteraction = () => {
		...
		const selectInteraction = new Select({
			...
			layers: layer => {
				const localState =
					this._model?.sharedModel.awareness.getLocalState();
		...
		switch (source.type) {
			case 'RasterSource': {
				const sourceParameters =
					source.parameters as IRasterSource;

				const pmTiles =
					sourceParameters.url.endsWith(
						'.pmtiles',
					);
				...
				if (!pmTiles) {
					newSource = new XYZSource({
						interpolate:
							sourceParameters.interpolate,
						attributions:
							sourceParameters.attribution,
			...
			case 'RasterDemSource': {
				const sourceParameters =
					source.parameters as IRasterDemSource;
			...
			case 'ShapefileSource': {
				const parameters =
					source.parameters as IShapefileSource;
			...
			case 'VideoSource': {
				console.warn(
					'Video Tiles not supported with Open Layers',
				);
			...
			case 'GeoTiffSource': {
				...
				const addNoData = (
					url: (typeof sourceParameters.urls)[0],
				) => {
					return { ...url, nodata: 0 };
				};
				const sources = await Promise.all(
					sourceParameters.urls.map(
						async sourceInfo => {
							const isRemote =
								sourceInfo.url?.startsWith(
									'http://',
								) ||
								sourceInfo.url?.startsWith(
									'https://',
	...
	private computeSourceUrl(source: IJGISSource): string {
		...
		if (url.includes('{max_zoom}')) {
			url = url.replace(
				'{max_zoom}',
				parameters.maxZoom.toString(),
			);
		}
		if (url.includes('{min_zoom}')) {
			url = url.replace(
				'{min_zoom}',
				parameters.minZoom.toString(),
			);
		}
	...
	private async _buildMapLayer(
		...
		if (layer.type !== 'StacLayer') {
			...
			source =
				this._model.sharedModel.getLayerSource(
					sourceId,
				);
			if (!source) {
				return;
		...
		switch (layer.type) {
			case 'RasterLayer': {
				layerParameters =
					layer.parameters as IRasterLayer;
			...
			case 'VectorLayer': {
				layerParameters =
					layer.parameters as IVectorLayer;
				...
				newMapLayer = new VectorLayer({
					...
					style: this.vectorLayerStyleRuleBuilder(
						layer,
					),
				});
			...
			case 'HillshadeLayer': {
				layerParameters =
					layer.parameters as IHillshadeLayer;
			...
			case 'ImageLayer': {
				layerParameters =
					layer.parameters as IImageLayer;
			...
			case 'WebGlLayer': {
				layerParameters =
					layer.parameters as IWebGlLayer;
			...
			case 'HeatmapLayer': {
				layerParameters =
					layer.parameters as IHeatmapLayer;
			...
			case 'StacLayer': {
				layerParameters =
					layer.parameters as IStacLayer;
	...
	addProjection(newMapLayer: Layer) {
		const sourceProjection = newMapLayer
			.getSource()
			?.getProjection();
		if (!sourceProjection) {
			console.warn(
				'Layer source projection is undefined or invalid',
			);
			return;
		}
	...
	async addLayer(
		...
		try {
			const newMapLayer = await this._buildMapLayer(
				id,
				layer,
			);
			if (newMapLayer !== undefined) {
				...
				const numLayers = this._Map
					.getLayers()
					.getLength();
				const safeIndex = Math.min(index, numLayers);
				this._Map
					.getLayers()
					.insertAt(safeIndex, newMapLayer);
			}
		} catch (error: any) {
			if (
	...
	vectorLayerStyleRuleBuilder = (layer: IJGISLayer) => {
		...
		if (
			...
			if (layer.filters.appliedFilters.length === 1) {
				...
				filterExpr = [
					layer.filters.logicalOp,
					...layer.filters.appliedFilters.map(
						buildCondition,
					),
				];
			}
	...
	async updateLayer(
		...
		switch (layer.type) {
			...
			case 'VectorTileLayer': {
				const layerParams =
					layer.parameters as IVectorTileLayer;
	...
	handleTemporalController = (id: string, layer: IJGISLayer) => {
		...
		if (layer.filters?.appliedFilters.length) {
			...
			const filteredFeatures = this._originalFeatures[
				id
			].filter(feature => {
				const featureTime = feature.get(
	...
	private flyToGeometry(sender: IJupyterGISModel, geometry: any): void {
		if (!geometry || typeof geometry.getExtent !== 'function') {
			console.warn(
				'Invalid geometry for flyToGeometry:',
				geometry,
			);
			return;
		}
	...
	private _onClientSharedStateChanged = (
		...
		if (remoteUser) {
			...
			if (
				...
				this.state.remoteUser?.username
			) {
				this.setState(old => ({
					...old,
			...
			if (remoteViewport.value) {
				const { x, y } =
					remoteViewport.value.coordinates;
			...
			if (this.state.remoteUser !== null) {
				...
				const viewportState =
					localState.viewportState?.value;
		...
		const isTemporalControllerActive =
			localState.isTemporalControllerActive;
	...
	private _onLayersChanged(
		...
		change.layerChange?.forEach(change => {
			...
			const mapLayer = this.getLayer(id);
			const layerTree = JupyterGISModel.getOrderedLayerIds(
				this._model,
			);
	...
	private _onSharedModelStateChange = (
		...
		const changedState = change.stateChange?.map(
			value => value.name,
		);
		...
		if (path !== this._documentPath && typeof path === 'string') {
			if (
				window.jupytergisMaps !== undefined &&
				this._documentPath
			) {
				delete window.jupytergisMaps[
					this._documentPath
				];
			}
			this._documentPath = path;
			if (window.jupytergisMaps !== undefined) {
				window.jupytergisMaps[this._documentPath] =
					this._Map;
			}
		}
	};
			...
			if (
				...
				if (typeof data === 'string') {
					try {
						...
						console.warn(
							`Failed to parse annotation data for ${key}:`,
							e,
						);
						return;
					}
	...
	private _updateAnnotation() {
		Object.keys(this.state.annotations).forEach(key => {
			...
			if (el) {
				const annotation =
					this._model.annotationModel?.getAnnotation(
						key,
					);
				if (annotation) {
					const screenPosition =
						this._computeAnnotationPosition(
							annotation,
						);
					if (screenPosition) {
	...
	private _onZoomToPosition(_: IJupyterGISModel, id: string) {
		...
		const annotation =
			this._model.annotationModel?.getAnnotation(id);
		if (annotation) {
			this._flyToPosition(
				annotation.position,
				annotation.zoom,
	...
	private _identifyFeature(e: MapBrowserEvent<any>) {
		...
		if (!selectedLayer) {
			console.warn(
				'Layer must be selected to use identify tool',
			);
			return;
		}
		...
		switch (jgisLayer?.type) {
			case 'WebGlLayer': {
				...
				const coordinate =
					this._Map.getCoordinateFromPixel(
						e.pixel,
					);
	...
	render(): JSX.Element {
		return (
			<>
				{Object.entries(this.state.annotations).map(
					([key, annotation]) => {
						if (
							!this._model
								.annotationModel
