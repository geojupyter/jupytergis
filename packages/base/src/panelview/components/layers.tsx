...
export class LayersPanel extends Panel {
	...
	private _onDrop = (e: DragEvent) => {
		...
		// element of a group.
		if (
			dragOverElement.classList.contains(
				LAYER_GROUP_HEADER_CLASS,
			) &&
...
function LayersBodyComponent(props: IBodyProps): JSX.Element {
	...
	useEffect(() => {
		...
		updateLayers();
		return () => {
			model?.sharedModel.layersChanged.disconnect(
				updateLayers,
			);
			model?.sharedModel.layerTreeChanged.disconnect(
				updateLayers,
			);
		};
	}, [model]);
...
const LayerGroupComponent: React.FC<ILayerGroupProps> = props => {
	...
	useEffect(() => {
		...
		const getExpandedState = async () => {
			const groupState = await state.fetch(
				`jupytergis:${group.name}`,
			);

			setOpen(
				((groupState as ReadonlyPartialJSONObject)
	...
	useEffect(() => {
		...
		gisModel?.clientStateChanged.connect(
